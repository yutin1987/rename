var PlayerCtrl, Q, QItem, resetQ, resetSum;
...
resetQ = function() {
  ...
  return ga('send', 'event', 'Assignment', 'reset', null, ++resetSum);
};
...
PlayerCtrl = function($scope, $cookieStore) {
  ...
  $scope.addBoy = function() {
    ...
    $scope.playerName = '';
    return ga('send', 'event', 'Player', 'Add', 'Boy', $scope.playerName);
  };
  $scope.addGirl = function() {
    ...
    $scope.playerName = '';
    return ga('send', 'event', 'Player', 'Add', 'Girl', $scope.playerName);
  };
  $scope.viewPlayer = function() {
    $scope.display = $scope.display === 'display-player' ? '' : 'display-player';
    return ga('send', 'event', 'Player', 'View');
  };
  $scope.delBoy = function() {
    ...
    angular.forEach(boys, function(boy) {
      if (boy.check) {
        num++;
        return ga('send', 'event', 'Player', 'Del', 'Boy', boy.name);
      } else {
        return $scope.boys.push(boy);
      }
    });
    $cookieStore.put('boys', $scope.boys);
    return ga('send', 'event', 'Player', 'Del', 'BoySum', num);
  };
  $scope.delGirl = function() {
    ...
    angular.forEach(girls, function(girl) {
      if (girl.check) {
        num++;
        return ga('send', 'event', 'Player', 'Del', 'Girl', girl.name);
      } else {
        return $scope.girls.push(girl);
      }
    });
    $cookieStore.put('girls', $scope.girls);
    return ga('send', 'event', 'Player', 'Del', 'GirlSum', num);
  };
  $scope.getQ4Boy = function() {
    ...
    assign = QItem.rand();
    $scope.assignment = assign.replace(/{player}/g, function() {
      return '<span class="girl">' + (player.rand().name || 'GIRL') + '</span>';
    });
    return ga('send', 'event', 'Assignment', 'assign', 'Boy', assign);
  };
  $scope.getQ4Girl = function() {
    ...
    return ga('send', 'event', 'Assignment', 'assign', 'Girl', assign);
  };
